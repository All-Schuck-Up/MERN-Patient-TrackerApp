...
export default class createSympotom extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:5000/patient/" + this.props.patientId)
      ...
      .then((res) => {
        ...
        this.setState({
          ...
          immediateAttention: res.data.patientEntry.map((el) =>
            el.immediateAttention.toString()
          ),
    ...
    return (
      <div>
        <table className="table">
          ...
          <tbody>{this.psList()}</tbody>
        </table>
        <UpdateDialog
          patientEntry={this.state.patientEntry}
