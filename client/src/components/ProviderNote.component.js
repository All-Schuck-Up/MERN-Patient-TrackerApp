...
class ProviderNote extends React.Component {
  ...
  notify = () =>
    toast.success("Note Added to the Patient's Profile!", {
      position: 'top-right',
      autoClose: 5000,
  ...
  addButton() {
    //adds doctor note to the database when button is clicked
    var today = new Date();
    var date =
      today.getFullYear() +
      '-' +
    ...
    axios
      .put(
        'http://localhost:5000/patientEntry/addDoctorNote/5ecb471af1741b0a4e6b993a',
        {
          isDoctor: true,
  ...
  scheduleMeeting() {
    //redirects to the Zoom home page for meetings
  ...
  render() {
    ...
    return (
      <div className='DoctorNote'>
        ...
        <Alert color='success'>
          Past Notes for {this.props.patientLastName}
        </Alert>
        {notes}
      </div>
    );
